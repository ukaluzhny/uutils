# -*- coding: utf-8 -*-
"""
    This module provides customized tools to work 
    with text, including Unicode. It also import * from re
    The first group of tools return a specific part of a string
    >>> text_e = 'I said: "it should be easy". Bye'
    >>> Word1(text_e) == 'I'
    True
    >>> Word2(text_e) == 'said'
    True
    >>> Word3(text_e) == '"it should be easy". Bye'
    True
    ...
    True
    >>> binstr(576, 11, True) == '01001000000'
    True
    ...
    >>> WordL(text_h)  == ''
    True
    >>> reverse_word_order( 'משמעות מיוחדת' ) == 'מיוחדת משמעות'
    True
    
    "Replace by Dictionary" class
    >>> d = ReD({'אין': 'not', 'זה': 'it'}, U)
    >>> d(text_h) == u'כלומר, not משמעות מיוחדת לביטוי "not זו כריתות"'
    True
    >>> del d['אין']
    >>> d(text_h) == text_h
    True
    ...
    >>> d(text_h) == u'כלומר, not משמעות מיוחדת לביטוי "not זו כריתות"'
    True
    >>> t = ['כלומר, א', 'ין', ' משמעות מ', 'יו', 'חדת לב', 'יט', 'ו', 'י ', '"א', 'ין', ' זו כר', 'ית', 'ות"']
    >>> splitted(text_h, ['י.']) == t
    True
"""
...
def hexstr(x, n = 0, bare = False):
    s = (hex(x)[2:]).replace("L", "")
    form = '{}' if bare or (n == 0 and x < 10) else '0x{}'
    return form.format(s.zfill(n))
def binstr(x, n = 0, bare = False):
    s = bin(x)[2:]
    form = '{}' if bare or (n == 0 and x < 2) else '0b{}'
    return form.format(s.zfill(n))
